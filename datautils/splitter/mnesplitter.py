...
def segmented_freq_data_splitter(summaries: list[sm.SummaryModel]):
    ...
    X_train_segmented_freq_data = np.concatenate([summary.signal.get_freq_segmented_data()[0] for summary in X_train])
    X_val_segmented_freq_data = np.concatenate([summary.signal.get_freq_segmented_data()[0] for summary in X_val])
