...
from sfe.sfe_utils import cross_correlation, normalization
import typing as t
...
class SignalTransform:
    ...
    def get_cross_correlogram(self, signal_ref: t.Optional[np.ndarray] = None, 
                              entire = False, normalize = False):
        ...
        signal_ref : numpy.ndarray
            ...
            If this parameter is None, an auto-cross-correlogram is generated.
        
        entire: bool
            If bool = True, a mirrored cross-correlogram is gerenated, i.e., it
            will contain 2 * len(signal) + 1. If bool = False, a cross-
            correlogram with a lenght len(signal) + 1 is generated.
            
        normalize: bool
            This parameter determines if signals must be normalized. If several
            cross-correlograms should be generated in an application 
            considering the same reference, it is recommended to previously 
            normalize the signals and keep this parameter as False to reduce 
        ...
        cc = None
        l = None
        s1 = np.copy(self.signal)
        
        if (signal_ref is None):
            s2 = np.copy(self.signal)
            s2 = None
        else:
            s2 = signal_ref
        ...
        if len(s1) == len(s2):
            ...
            cc = [cross_correlation(s1, signal_ref, i) for i in l]
    ...
    def __get_psd(self, sig, data_taper = None, eigen = None):
        ...
        if (data_taper is not None) and (eigen is not None):
            ...
            ps, w, _ = pmtm(self.signal, NW = self.nw, k = self.k, 
                            show = False)
